...
<section id="kontak" class="py-24 md:py-32 bg-[#FAF8F4] relative overflow-hidden">
    ...
    {{-- Decorative --}}
    <div class="absolute top-0 right-0 w-96 h-96 rounded-full bg-[#C9A84C]/5 -translate-y-1/3 translate-x-1/3 pointer-events-none"></div>
    <div class="absolute bottom-0 left-0 w-64 h-64 rounded-full bg-[#C9A84C]/5 translate-y-1/3 -translate-x-1/3 pointer-events-none"></div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="space-y-5">
                        @php
                            $contacts = [
                                ...
                                [
                                    ...
                                    'fill'  => true,
                                    'href'  => 'https://www.instagram.com/kyh.organizer',
                                ],
                                [
                                    'label' => 'Email',
                        ...
                        @foreach ($contacts as $c)
                            <div class="flex items-start gap-4">
                                <div class="w-10 h-10 rounded-xl bg-[#C9A84C]/15 border border-[#C9A84C]/25 flex items-center justify-center flex-shrink-0">
                                    <svg class="w-4 h-4 text-[#C9A84C]"
                                        @if($c['fill']) fill="currentColor" @else fill="none" stroke="currentColor" stroke-width="1.5" @endif
                                        viewBox="0 0 24 24">
                                        {!! $c['icon'] !!}
                                    </svg>
                                </div>
                                <div>
                                    ...
                                    @if($c['href'])
                                        <a href="{{ $c['href'] }}" target="_blank" class="text-white font-medium text-sm hover:text-[#C9A84C] transition-colors duration-200">
                                            {{ $c['value'] }}
                                        </a>
                                    @else
                ...
                {{-- Quick WhatsApp CTA --}}
                <a
                    href="https://wa.me/6281234567890?text=Halo%20KYH%2C%20saya%20ingin%20konsultasi%20gratis"
                    target="_blank"
                    id="kontak-whatsapp-cta"
            ...
            <div class="lg:col-span-3">
                <form
                    id="kontak-form"
                    class="bg-white rounded-2xl p-8 md:p-10 shadow-xl shadow-black/5 border border-[#C9A84C]/10"
                    x-data="{
                        form: { nama: '', email: '', acara: '', pesan: '' },
                        sent: false,
                        submit() {
                            // Redirect to WhatsApp with prefilled message
                            const wa = `https://wa.me/6281234567890?text=Halo+KYH!+%0A%0ANama%3A+${encodeURIComponent(this.form.nama)}%0AEmail%3A+${encodeURIComponent(this.form.email)}%0AJenis+Acara%3A+${encodeURIComponent(this.form.acara)}%0APesan%3A+${encodeURIComponent(this.form.pesan)}`;
                ...
                >
                    <h3
                        class="text-2xl font-bold text-[#0D0D0D] mb-2"
                        style="font-family:'Playfair Display',serif;"
                    >
                        Kirim Pesan
                    </h3>
                    <p class="text-[#0D0D0D]/50 text-sm mb-8">Isi form di bawah dan kami akan menghubungi Anda segera.</p>

                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                        {{-- Nama --}}
                        <div>
                            <label class="block text-xs font-semibold tracking-widest uppercase text-[#0D0D0D]/60 mb-2" for="form-nama">Nama Lengkap</label>
                            <input
                                id="form-nama"
                                type="text"
                                x-model="form.nama"
                        ...
                        {{-- Email --}}
                        <div>
                            <label class="block text-xs font-semibold tracking-widest uppercase text-[#0D0D0D]/60 mb-2" for="form-email">Email</label>
                            <input
                                id="form-email"
                                type="email"
                                x-model="form.email"
                    ...
                    {{-- Jenis Acara --}}
                    <div class="mb-5">
                        <label class="block text-xs font-semibold tracking-widest uppercase text-[#0D0D0D]/60 mb-2" for="form-acara">Jenis Acara</label>
                        <select
                            id="form-acara"
                            x-model="form.acara"
                            required
                    ...
                    {{-- Pesan --}}
                    <div class="mb-8">
                        <label class="block text-xs font-semibold tracking-widest uppercase text-[#0D0D0D]/60 mb-2" for="form-pesan">Ceritakan Acara Anda</label>
                        <textarea
                            id="form-pesan"
                            x-model="form.pesan"
                            rows="4"
                            placeholder="Tanggal acara, estimasi tamu, konsep yang diinginkan..."
                            class="w-full px-4 py-3 rounded-xl border border-[#0D0D0D]/15 text-[#0D0D0D] text-sm
                    ...
                    <button
                        type="submit"
                        class="w-full py-4 rounded-xl bg-gradient-to-r from-[#C9A84C] to-[#A07830] text-[#0D0D0D]
                               font-bold text-sm tracking-widest uppercase
                               hover:from-[#E2BE7A] hover:to-[#C9A84C]
                    ...
                    <p class="text-center text-xs text-[#0D0D0D]/35 mt-4">
                        Atau hubungi langsung via WhatsApp: <a href="https://wa.me/6281234567890" target="_blank" class="text-[#C9A84C] hover:underline">+62 812-3456-7890</a>
                    </p>
                </form>
            </div>
